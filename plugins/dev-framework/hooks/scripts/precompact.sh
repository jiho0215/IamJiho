#!/bin/bash
# precompact.sh — PreCompact hook: serialize pipeline state before context truncation
# Safety: ERR trap ensures unexpected errors never block compaction

trap 'exit 0' ERR
set -uo pipefail

command -v jq &>/dev/null || exit 0

CONFIG="$HOME/.claude/autodev/config.json"
cfg() {
  if [ -f "$CONFIG" ]; then
    local val
    val=$(jq -r "($1) // empty" "$CONFIG" 2>/dev/null)
    if [ -n "$val" ]; then echo "$val"; else echo "$2"; fi
  else
    echo "$2"
  fi
}

sanitize_branch() {
  echo "$1" | sed 's|[/\\:*?"<>|@]|-|g' | sed 's|\.\.*$||' | cut -c1-64
}

SESSIONS_DIR=$(cfg '.paths.sessionsDir' "$HOME/.claude/autodev/sessions")
BRANCH=$(git symbolic-ref --short HEAD 2>/dev/null || echo "unknown")
REPO=$(basename "$(git remote get-url origin 2>/dev/null \
    || git rev-parse --show-toplevel 2>/dev/null \
    || pwd)" .git)
SANITIZED_BRANCH=$(sanitize_branch "$BRANCH")
SESSION_FORMAT=$(cfg '.sessionFolderFormat' '{repo}--{branch}')
SESSION_NAME="${SESSION_FORMAT/\{repo\}/$REPO}"
SESSION_NAME="${SESSION_NAME/\{branch\}/$SANITIZED_BRANCH}"
SESSION_DIR="$SESSIONS_DIR/$SESSION_NAME"

# Only inject state if an active pipeline session exists
[ -f "$SESSION_DIR/progress-log.json" ] || exit 0

CURRENT_PHASE=$(jq -r '.currentPhase // "unknown"' "$SESSION_DIR/progress-log.json" 2>/dev/null)
STATUS=$(jq -r '.status // "unknown"' "$SESSION_DIR/progress-log.json" 2>/dev/null)
TICKET=$(jq -r '.ticket // "unknown"' "$SESSION_DIR/progress-log.json" 2>/dev/null)
ISSUES=$(jq -r '.summary.totalIssuesFound // 0' "$SESSION_DIR/progress-log.json" 2>/dev/null)
DECISIONS=$(jq -r '.decisions | length' "$SESSION_DIR/decision-log.json" 2>/dev/null || echo "0")

cat <<EOF
--- PIPELINE STATE (preserved before context compaction) ---
Ticket: $TICKET | Phase: $CURRENT_PHASE | Status: $STATUS
Issues found: $ISSUES | Decisions logged: $DECISIONS
Session: $SESSION_DIR
Resume: /dev-pipeline $TICKET --from $CURRENT_PHASE
Full status: /dev-pipeline $TICKET --status
--- END PIPELINE STATE ---
EOF

exit 0
